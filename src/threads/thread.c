...
void thread_wait(int64_t ticks)
{
  struct thread *t = thread_current ();
  t->wakeup_time = ticks;
  list_insert_order (&wait_list, &t->elem, thread_greater, NULL);
  intr_disable();
  thread_block();
  intr_enable();
}
...
thread_unblock (struct thread *t) 
{
  ...
  list_insert_order (&ready_list, &t->elem, thread_greater, NULL);
...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread) 
    list_insert_order (&ready_list, &cur->elem, thread_greater, NULL);
